...
export default function Dashboard(props) {
    const {proveedores, productos, detalle_compras} = usePage().props;
    ...
    function handlePedidoSubmit(e) {
        e.preventDefault();
        // console.log(data);
        postPedido(route("compras.store"));
    }
    ...
    useEffect(() => {
        ...
        const nuevoIGV = (pedidoData.subtotal) * 0.18;
        ...
        if (pedidoData.monto_pagado >= totalAPagar) {
             setPedidoData(prevData => ({...prevData, estado: "Cancelado"}));
         } else {
             setPedidoData(prevData => ({...prevData, estado: "Pendiente"}));
         }

    }, [pedidoData.impuestos, pedidoData.monto_pagado, pedidoData.total]);
    ...
    async function handleDetallePedidoSubmit(e) {
        e.preventDefault();
        try {
            const response = await axios.post(route("compradetalles.store"), detallePedidoData);
                            ...
                            <div className="lg:flex block">
                                <div className="flex flex-col w-full mr-4">
                                    ...
                                    <div className="mt-4">
                                        ...
                                        <select
                                            //value={detallePedidoData.metodo_pago}
                                            onChange={(e) => {
